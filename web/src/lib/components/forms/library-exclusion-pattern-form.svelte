...
<FullScreenModal on:clickOutside={() => handleCancel()}>
  ...
  >
    ...
    <form on:submit|preventDefault={() => handleSubmit()} autocomplete="off">
      <p class="p-5 text-sm">
        排除模式允许您在扫描库时忽略文件和文件夹。这在您有包含不希望导入的文件的文件夹（例如原始文件）时非常有用。
        <br /><br />
		添加排除模式。支持使用 *，** 和 ? 进行模式匹配。要忽略名为 "Raw" 的任何目录中的所有文件，请使用 "**/Raw/**"。
		要忽略以 ".tif" 结尾的所有文件，请使用 "**/*.tif"。要忽略绝对路径，请使用 "/path/to/ignore"。
      </p>
